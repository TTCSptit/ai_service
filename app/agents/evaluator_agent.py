...
from app.core.llm import get_llm_cheap, get_llm_vip
...
COMBINED_TOOLS = INTERVIEW_TOOLS + [execute_code_sandbox]
...
class TechLeadEvaluator:
    async def generate_draft(self, history, system_prompt: str, user_prompt: str) -> str:
        llm_with_tools = get_llm_cheap().bind_tools(COMBINED_TOOLS)
        tool_check_messages = history + [
            SystemMessage(content=system_prompt),
            HumanMessage(content=user_prompt + "\n\n(LƯU Ý QUAN TRỌNG: NẾU ứng viên gửi code giải bài tập, BẮT BUỘC phải dùng các tool như `execute_code_sandbox` hoặc `analyze_candidate_code` để chạy thử và lấy báo cáo trước khi trả lời!)")
        ]
        ...
        try:
            ...
            if ai_msg.tool_calls:
                ...
                logger.info("[Agent Groq] Đã có báo cáo Sandbox.")
        ...
        try:
            ...
            final_draft = await get_llm_cheap().ainvoke(messages)
    ...
    async def revise_draft(self, history, system_prompt: str, user_prompt: str, old_draft: str, feedback: str) -> str:
        ...
        try:
            revised_res = await get_llm_cheap().ainvoke(messages)
